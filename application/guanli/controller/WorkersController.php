...
class WorkersController extends BasicController {
	...
	public function save(){
		...
		$workerfield = array('id','name', 'password','type','group');
		$workermodel=new Workers();
		$workerData = array();
		...
		if(isset($data['id']) && $data['id']){//如果存在id，更新操作
			//更新基本信息
		    $result = $workermodel->update($workerData, ['id'=>(int)$data['id']]);
		}else{
	...
	public function getgroupmenus() {
	    ...
	    $group = new WorkersGroup();
	    $result = $group->get(['id'=>(int)$group_id]);
	...
	public function addmenutogroup() {
	    ...
	    if(isset($data['id']) && $data['id']){//如果存在id，更新操作
	        //更新基本信息
	        $result = $workergroupmodel->update($data, ['id'=>(int)$data['id']]);
	    }else{
